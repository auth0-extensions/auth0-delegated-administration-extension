...
function attachStorageHelpers(context) {
    ...
    function readFromPath(path, options, cb) {
        ...
        request
            ...
            // Treat any HTTP status as a resolved response so the branching below mirrors the original `request` behaviour.
            .ok(function () { return true; })
            .end(function (err, res) {
                if (err || !res) return cb(Boom.boomify(err || new Error('No response'), { statusCode: 502 }));

                var body = res.body;
                if (res.status === 404 && Object.hasOwnProperty.call(options, 'defaultValue')) return cb(null, options.defaultValue);
                if (res.status >= 400) return cb(new Boom(body && body.message, { statusCode: res.status }));
